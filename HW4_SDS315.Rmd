...
ggplot(Highway_average_change) + geom_histogram(aes(x = mean), binwidth = 0.005) + labs(x = "avergae mean differnece in price", y = "frequency", title = "Average price difference between gas stations at highways and gas stations not at highways")
```
...
```
The 95% confidence interval is `r round(Ed_vs_Earl_ci[3],3)` to `r round(Ed_vs_Earl_ci[4],3)`.
...
```
The 95% confidence interval is `r round(Loser_vs_Apprentice_ci[3],3)` to `r round(Loser_vs_Apprentice_ci[4],3)`.
...
Approach: Bootstrap the proportion of results that responded a 4 or higher.
...
Results: The 95% confidence interval is `r round(ad_differnce_ci[3],3)` to `r round(ad_differnce_ci[4],3)`.
